...
const Masalah = ({ user, addPengajuan }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        addPengajuan(data.newPengajuan); 
        navigate('/hasil'); 
      } else {
    ...
    <>
      <div className='title-info'>
        <h1 className='title-info'>
          <span className='bold-text'>KTM bermasalah | </span>
          <span className='italic-text'>Informasi dan Syarat Bagi KTM yang masih bermasalah</span>
        </h1>
      </div>
